...
- Prisma·Render·CI 모두 **`DATABASE_URL` 하나만** 사용합니다(별도 `DIRECT_URL` 없음).
- Transaction pooler(6543)만 쓰면 `migrate` / `db push`가 **멈추거나 실패**할 수 있어, **가능하면 Session pooler(5432)** 또는 Direct(5432) URI를 `DATABASE_URL`로 두는 것을 권장합니다.
- 로컬에서도 Postgres를 쓰고 싶으면 Supabase **같은 프로젝트**에 연결하거나, PC에 PostgreSQL을 설치해 `DATABASE_URL`만 맞추면 됩니다.
...
| 구분 | 용도 (이 프로젝트 기준) |
|------|-------------------------|
| **Direct** (`db.*.supabase.co`, 포트 **5432**) | `DATABASE_URL`로 쓰면 DDL·마이그레이션에 적합 |
| **Session pooler** (같은 pooler 호스트, 포트 **5432**) | Direct가 막힐 때 `DATABASE_URL` 후보로 흔히 사용 |
| **Transaction pooler** (포트 **6543**, `pgbouncer=true`) | 연결 다발에 유리하지만, **Prisma `migrate` / `db push`에만 쓰면 장시간 무반응**일 수 있음 |
...
**이 저장소 스키마**는 `DATABASE_URL`만 씁니다. Supabase에서 **Session pooler(5432)** URI를 `DATABASE_URL`로 두면 빌드(`migrate deploy`)와 런타임이 모두 단순해집니다. 트래픽이 매우 많으면 나중에 풀러 전략을 다시 조정하면 됩니다.

예시(Session, 값은 본인 프로젝트 것으로 교체):

```text
postgresql://postgres.xxxxx:[YOUR-PASSWORD]@aws-0-ap-northeast-1.pooler.supabase.com:5432/postgres?sslmode=require
```

- 비밀번호에 특수문자가 있으면 **URL 인코딩**이 필요할 수 있습니다.
- 루트의 `server.env` 등은 Express/Prisma가 읽지 않습니다. **`cd server` 뒤** `server/.env`만 로드됩니다.
...
1. `server/.env` 에 `DATABASE_URL`(Supabase는 Session **5432** 권장)을 설정합니다.
...
- Supabase **`DATABASE_URL`**(가능하면 Session pooler **5432**), 강한 **`JWT_SECRET`**, 프론트 배포 후 확정되는 **프론트 URL**.
...
4. **Build Command** 예시:
   ...
   (`build:render` = `prisma generate` + `migrate deploy` + `tsc`)
...
6. **Environment** 에 다음을 추가:
   ...
   | Key | 값 |
   |-----|-----|
   | `DATABASE_URL` | Supabase Postgres URI(가능하면 **Session pooler `:5432`**) |
...
- [ ] `DATABASE_URL` · `.env` 가 Git 저장소에 올라가지 않았다 (`.gitignore` 확인).
...
- **조치**: `DATABASE_URL`을 **Session pooler(5432)** 또는 Direct(5432)로 바꾼 뒤 다시 실행합니다.
...
```env
DATABASE_URL="postgresql://postgres.xxxxx:비밀번호@....pooler.supabase.com:5432/postgres?sslmode=require"
