...
- 로컬에서도 Postgres를 쓰고 싶으면, Supabase의 **같은 프로젝트**에 연결하거나, PC에 PostgreSQL을 설치해 `DATABASE_URL`만 바꿔 테스트할 수 있습니다.
...
| 구분 | 용도 (이 프로젝트 기준) |
|------|-------------------------|
| **Direct** (보통 포트 **5432**) | 짧은 작업, 로컬에서 `migrate dev`, 일부 배치 |
| **Transaction pooler** (보통 포트 **6543**) | **웹 서버(Express)처럼 동시 접속이 많은 경우**에 적합 |
...
**로컬에서 `prisma migrate dev`** 를 돌릴 때 Pooler에서 오류가 나면, Supabase가 안내하는 대로 **Direct(5432)** URL로 마이그레이션만 실행하는 방법도 있습니다.

SSL이 필요한 경우 연결 문자열에 `sslmode=require` 가 포함되거나, Supabase 기본 URI에 이미 포함되는 경우가 많습니다. 오류 메시지에 `SSL` 이 보이면 Prisma/Supabase FAQ를 참고하세요.
...
1. `server/.env` 에 `DATABASE_URL`을 Supabase Postgres URI로 설정합니다.
...
- Supabase `DATABASE_URL` (Pooler 권장), 강한 **`JWT_SECRET`**, 프론트 배포 후 확정되는 **프론트 URL**.
...
6. **Environment** 에 다음을 추가:
   ...
   | Key | 값 |
   |-----|-----|
   | `DATABASE_URL` | Supabase Postgres URI (Pooler + Prisma 권장 옵션) |
